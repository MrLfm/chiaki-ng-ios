 ...
 * UDP Hole Punching Implementation
 “通过互联网进行远程游戏”使用自定义的基于 UDP的协议在控制台和客户端之间进行通信（有关详细信息，请参阅`rudp.h`）。该协议旨在即使控制台和客户端都位于 NAT 后面，也能通过中间服务器使用 UDP 打洞来工作。此文件使用 PSN API 实现打洞逻辑。
...
{
    ChiakiErrorCode err = get_websocket_fqdn(session, &session->ws_fqdn);// 创建ws_fqdn，如："44-229-233-87-pushcl.np.communication.playstation.net"
    if (err != CHIAKI_ERR_SUCCESS)
        return err;
    chiaki_mutex_lock(&session->stop_mutex);
...
/**
 * 获取我们可以从中获取 PSN 通知的 websocket 服务器的完全限定域名。
...
static void* websocket_thread_func(void *user) {
    ...
    /**
     400：缺少必要参数
     403：服务器拒绝了请求，通常是因为权限问题或请求头不正确
     */
    struct curl_slist *headers = NULL;
    headers = curl_slist_append(headers, session->oauth_header);// Authorization: Bearer 7d5e7154-bb48-46a7-99c0-3c30719a0a1f
    headers = curl_slist_append(headers, "Sec-WebSocket-Protocol: np-pushpacket");// 必不可少参数
    headers = curl_slist_append(headers, "User-Agent: WebSocket++/0.8.2");
    headers = curl_slist_append(headers, "X-PSN-APP-TYPE: REMOTE_PLAY");// 必不可少参数
    headers = curl_slist_append(headers, "X-PSN-APP-VER: RemotePlay/1.0");// 必不可少参数
    headers = curl_slist_append(headers, "X-PSN-KEEP-ALIVE-STATUS-TYPE: 3");
    headers = curl_slist_append(headers, "X-PSN-OS-VER: Windows/11.0");// 必不可少参数
    headers = curl_slist_append(headers, "X-PSN-PROTOCOL-VERSION: 2.1");// 必不可少参数
    ...
    if (res != CURLE_OK)// CURLE_HTTP_RETURNED_ERROR
    {
        if (res == CURLE_HTTP_RETURNED_ERROR)
        {
            long http_code = 0;
            ...
            CHIAKI_LOGE(session->log, "websocket_thread_func: Connecting to push notification WebSocket %s failed with CURL error %s, code: %d", ws_url, curl_easy_strerror(res), res);
        }
        goto cleanup;
    }
