...
CHIAKI_EXPORT ChiakiErrorCode chiaki_stream_connection_run(ChiakiStreamConnection *stream_connection, chiaki_socket_t *socket)
{
	...
	takion_info.cb = stream_connection_takion_cb;// 这个cb在chiaki_takion_connect()方法赋值给takion.cb
...
static void stream_connection_takion_cb(ChiakiTakionEvent *event, void *user)
{
//    printf("stream_connection_takion_cb - 收到响应类型：%u\n", event->type);
...
//                event.quit.reason = CHIAKI_QUIT_REASON_STREAM_CONNECTION_UNKNOWN;
//                event.quit.reason_str = "网络异常";
...
static void stream_connection_takion_av(ChiakiStreamConnection *stream_connection, ChiakiTakionAVPacket *packet)
{
    chiaki_gkcrypt_decrypt(stream_connection->gkcrypt_remote, packet->key_pos + CHIAKI_GKCRYPT_BLOCK_SIZE, packet->data, packet->data_size);// 解密音视频、体感数据
    
    if(packet->is_video) {
//        printf("收到视频数据\n");
        chiaki_video_receiver_av_packet(stream_connection->video_receiver, packet);
    }
    else if (packet->is_haptics) {// 体感数据，连接时设置enable_dualsense为true才会收到，否则收不到
//        printf("收到体感数据\n");
