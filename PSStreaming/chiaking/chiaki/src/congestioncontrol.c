...
static void *congestion_control_thread_func(void *user)
{
	...
	while(true)
	{
		...
		CHIAKI_LOGV(control->takion->log, "Sending Congestion Control Packet, received: %u, lost: %u",
			(unsigned int)packet.received, (unsigned int)packet.lost);// 问题：长时间连接后，出现接收数据长度为0的情况
        err = chiaki_takion_send_congestion(control->takion, &packet);
//        if (err == CHIAKI_ERR_NETWORK
//            || err == CHIAKI_ERR_TIMEOUT) {// TODO: 可能CHIAKI_ERR_TIMEOUT不合适？
//            if (control->takion->cb) {
//                ChiakiTakionEvent event = {
//                    .type = CHIAKI_TAKION_EVENT_TYPE_DISCONNECT
//                };
//                control->takion->cb(&event, control->takion->cb_user); // 💥 上报断开事件，将会执行：stream_connection_takion_cb
//            }
//        }
	}
