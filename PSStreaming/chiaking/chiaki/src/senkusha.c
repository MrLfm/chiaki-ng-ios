...
static void senkusha_takion_extra_cb(ChiakiTakionEvent *event, void *user);
...
#define QUIT(quit_label) do { \
	...
	senkusha->state_finished = false;
	senkusha->state_failed = false;
	err = chiaki_takion_connect(&senkusha->takion, &takion_info, socket);// 这里的takion是senkusha新建的
	if(!socket)
		free(takion_info.sa);
...
static void senkusha_takion_cb(ChiakiTakionEvent *event, void *user)
{
//    printf("senkusha_takion_cb - 收到响应类型：%u\n", event->type);
	ChiakiSenkusha *senkusha = user;
	switch(event->type)
	{
