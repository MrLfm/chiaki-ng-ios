...
CHIAKI_EXPORT ChiakiErrorCode chiaki_takion_connect(ChiakiTakion *takion, ChiakiTakionConnectInfo *info, chiaki_socket_t *sock)
{
	...
	takion->cb = info->cb;// 设置视频数据回调，指向：stream_connection_takion_cb
	...
	if(sock)
	{
        ...
        size_t size;
        char sysver[128];
        sysctlbyname("kern.osrelease", sysver, &size, NULL, 0);
        if (sscanf(sysver, "%d", &majorVersion) == 1) {
...
{  
	...
	if(err != CHIAKI_ERR_SUCCESS)
	{
		CHIAKI_LOGE(takion->log, "Takion failed to calculate mac for received packet");// 长时间连接后，走这里
        printf("接收数据错误：%u\n", err);
		return err;
	}
...
static void takion_handle_packet_av(ChiakiTakion *takion, uint8_t base_type, uint8_t *buf, size_t buf_size)
{
    ...
    // 这里的cb刚开始由senkusha持有，后面由session的session_thread_func的chiaki_stream_connection_run方法更换为stream_connection_takion_cb
	if(takion->cb)
	{
		ChiakiTakionEvent event = { 0 };
