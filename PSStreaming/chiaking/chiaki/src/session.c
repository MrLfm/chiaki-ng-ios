	...
	ChiakiLog *log)
{
	...
	err = chiaki_stream_connection_init(&session->stream_connection, session, connect_info->packet_loss_max);// 创建stream_connection
	if(err != CHIAKI_ERR_SUCCESS)
	{
...
ctrl_failed:
	...
	ChiakiSenkusha senkusha;// 新建ChiakiSenkusha
    ...
    printf("================== session_thread_func ==================\n");
	chiaki_mutex_unlock(&session->state_mutex);
	err = chiaki_stream_connection_run(&session->stream_connection, data_sock);// finish session后不走这里
	chiaki_mutex_lock(&session->state_mutex);
...
quit:
	...
	chiaki_mutex_unlock(&session->state_mutex);
	chiaki_session_send_event(session, &quit_event);// 如果主机已被连接，第一次没有执行这步
	return NULL;
...
static ChiakiErrorCode session_thread_request_session(ChiakiSession *session, ChiakiTarget *target_out)
{
	...
	else
	{
		for(struct addrinfo *ai=session->connect_info.host_addrinfos; ai; ai=ai->ai_next)
		{
			...
			set_port(sa, htons(SESSION_PORT));// 设置端口：9295

			// TODO: this can block, make cancelable somehow
			int r = getnameinfo(sa, (socklen_t)ai->ai_addrlen, session->connect_info.hostname, sizeof(session->connect_info.hostname), NULL, 0, NI_NUMERICHOST);// session->connect_info.hostname == 192.168.31.211？？
			if(r != 0)
			{
			...
			CHIAKI_LOGI(session->log, "Trying to request session from %s:%d", session->connect_info.hostname, SESSION_PORT);// 192.168.31.211:9295

			session_sock = socket(ai->ai_family, SOCK_STREAM, 0);// (2, 1, 0) -> 9
