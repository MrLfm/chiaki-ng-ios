...
CHIAKI_EXPORT ChiakiErrorCode chiaki_gkcrypt_gmac(ChiakiGKCrypt *gkcrypt, uint64_t key_pos, const uint8_t *buf, size_t buf_size, uint8_t *gmac_out)
{
    ...
    // 加载 default provider（OpenSSL 3.x 必须）
//    OSSL_PROVIDER *provider = OSSL_PROVIDER_load(NULL, "default");// 崩溃
//    if (!provider)
//    {
//        printf("加载 default provider 失败\n");
...
//    }
    ...
    if(!ctx)
    {
        printf("EVP_CIPHER_CTX_new 失败\n");
        ret = CHIAKI_ERR_MEMORY;
        goto fail;
    }
//    else {
//        printf("EVP_CIPHER_CTX_new 成功\n");
//    }
    ...
    if (!cipher)
    {
        printf("EVP_aes_128_gcm 返回 NULL\n");
        ret = CHIAKI_ERR_UNKNOWN;
        goto fail;
    }
//    else {
//        printf("EVP_aes_128_gcm 返回有值\n");
//    }
    
    // 添加一行测试 cipher 名字（看看这个 EVP_CIPHER 实例是否完整）
//    printf("cipher name: %s\n", EVP_CIPHER_name(cipher));
    
    if(!EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, 1))
    {
        unsigned long err = ERR_get_error(); // 获取错误代码
        char err_buf[256];
        ERR_error_string_n(err, err_buf, sizeof(err_buf)); // 转换成可读字符串
        
        printf("EVP_CipherInit_ex 失败，OpenSSL 错误码：%lu，错误信息：%s\n", err, err_buf);
        ret = CHIAKI_ERR_UNKNOWN;
        goto fail_cipher;
    }
