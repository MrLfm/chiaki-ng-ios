...
//    printf("111 Regist Info: target=%d, host=%s, pin=%d\n", info->target, info->host, info->pin);
...
//    printf("222 Regist Info: target=%d, host=%s, pin=%d\n", regist->info.target, regist->info.host, regist->info.pin);
...
static ChiakiErrorCode regist_search(ChiakiRegist *regist, struct addrinfo *addrinfos, struct sockaddr *recv_addr, socklen_t *recv_addr_size)
{
	CHIAKI_LOGI(regist->log, "Regist starting search");// 崩溃在这？
	...
	if(regist->info.broadcast)
		r = sendto_broadcast(regist->log, sock, src, strlen(src) + 1, 0, (struct sockaddr *)&send_addr, send_addr_len);// TODO: r == -1
	else
		r = send(sock, src, strlen(src) + 1, 0);// 返回5
	if(r < 0)
	{
